# Integração React/shadcn: Background Paths

Este projeto atual é Django puro, com templates HTML e CSS. Ele não possui estrutura React, Tailwind CSS, TypeScript ou shadcn.

Para usar o componente `background-paths.tsx`, crie uma frente React/Next separada ou migre a interface para Next.js.

## Setup recomendado

```bash
npx create-next-app@latest vestlink-web --typescript --tailwind --eslint --app
cd vestlink-web
...
O caminho padrão de componentes shadcn deve ser:

```text
components/ui
```

Esse caminho é importante porque os imports usam:
...
Se o projeto não tiver `/components/ui`, crie essa pasta ou ajuste os aliases/imports.

## Arquivo: components/ui/background-paths.tsx
